 ...
 * Вычисляет массив любой длины, допустимой видеокартой (для CC3.0+ это (2^31 −
 * 1)*2^10 ≈ 2.1990233e+12 значений)
 *
 ...
 *
 * \return noise – (см. описание параметра) функция изменяет переданный массив
 * (хранится в памяти GPU).
 * \return octave – (см. описание параметра) функция изменяет переданный массив
 * (хранится в памяти GPU).
 */
template <typename T>
__global__ void Perlin1D_kernel(T *noise, T *octave, const T *k, uint32_t size,
                                T step, uint32_t numSteps,
                                bool isOctaveCalkNeed) {
  uint32_t id =
      blockIdx.x * blockDim.x + threadIdx.x; // [0..] – всего точек для просчёта
  if (id >= size)
    return;
  uint32_t n = id * step; // 0 0 0 / 1 1 1 / 2 2 2 / .. – какие точки к каким
                          // контрольным точкам принадлежат
  uint32_t dotNum = id % numSteps; // 0 1 2 / 0 1 2 / 0 1 2 / .. – позиция точки
                                   // между левым и правым значением
  T t = dotNum * step; // 0.0 0.33 0.66 / 0 0.33 0.66 / .. – численное значение
                       // точки для интерполяции
  t = smoothstep_kernel<T>(t); // Применяем сигмоидальную(на промежутке [0, 1])
                               // функцию, реализуя градиент
  T y0 = k[n] * t;             // kx+b (b = 0)
  T y1 = k[n + 1] * (t - 1);   // kx+b (b = -k) = k(x-1)
  noise[id] = lerp_kernel<T>(
      y0, y1, t); // Интерполяцией находим шум, пишем сразу в выходной массив
...
/**
 * Вспомогательная функция для вычисления шума Перлина на оси.
 *
 * @param noise Массив с результатом вычисления шума перлина на оси.
 * @param k Массив со значениями наклона уравнений в контрольных узлах.
 * @param step Величина шага между точками, в которых вычисляется шум.
...
template <typename T> void Perlin1D(T *vertices, const Params1D &p) {
  ...
  T *dev_k =
      nullptr; // pointer to array with tilt angle (tg slope angle) in VRAM
  ...
  cudaMemcpy(dev_k, p.k_.data(), p.controlPoints * sizeof(T),
  ...
  Perlin1D_kernel<T><<<blocksPerGrid, threadsPerBlock>>>(
      dev_noise, dev_octave, dev_k, p.resultDotsCols, p.step, p.numSteps,
      static_cast<bool>(p.octaveNum));

  std::function<void(void)> sharedKernel;
  // Выполняем наложение октав на получившийся шум.
  if (p.octaveNum) {
    if (p.resultDotsCols <=
        2 * 32 * 1024 / sizeof(T)) // если вся октава помещается в разделяемую
                                   // память, вызываем простое ядро
      Perlin1Doctave_shared_kernel<T><<<blocksPerGrid, threadsPerBlock>>>(
          dev_noise, dev_octave, p.resultDotsCols, p.octaveNum);
    else // если октава не помещается целиком, вызываем сложное ядро.
      Perlin1Doctave_shared_unlimited_kernel<T>
          <<<blocksPerGrid, threadsPerBlock>>>(dev_noise, dev_octave,
                                               p.resultDotsCols, p.octaveNum);
  }
  ...
  cudaMemcpy(vertices, dev_vertices, 2 * (p.resultDotsCols + 1) * sizeof(T),
